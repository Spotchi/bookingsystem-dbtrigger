...
export function detectLanguage(languageCode: string | null | undefined): SupportedLanguage {
  ...
  if (normalizedCode.startsWith('fr')) {
    return 'fr';
  } else if (normalizedCode.startsWith('nl') || normalizedCode.startsWith('du')) {
    return 'nl';
  } else {
...
export function getTemplateFromLanguageCode(languageCode: string | null | undefined): EmailTemplate {
